...
import { DEFINITION_TYPES } from '../types/contentBoxTypes';
...
/**
 * セクションをロードし、指定した border クラスの ContentBox だけを
 * 抽出して表示するコンポーネント。
 */
        ...
        const extract = () => {
            ...
            boxes.forEach((box, index) => {
                ...
                if (hasMatch) {
                    ...
                    link.className = 'text-sm font-medium text-slate-500 hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-400 transition-colors flex items-center gap-1 cursor-pointer';
        ...
        <>
            ...
            {!isReady && (
                <div className="flex justify-center py-3">
                    <div className="animate-spin w-5 h-5 rounded-full border-2 border-blue-100 border-t-blue-500 dark:border-slate-700 dark:border-t-blue-400" />
                </div>
            )}
            <div
                ref={displayRef}
...
export default function DefinitionListPage() {
    ...
    const definitions = useMemo(() => {
        return allContentBoxData.filter(
            item => item.topicId === topicId && (DEFINITION_TYPES as readonly string[]).includes(item.type)
        );
    }, [topicId]);
    ...
    const sectionsByChapter = useMemo(() => {
        ...
        for (const def of definitions) {
            if (!chapterMap[def.chapterId]) {
                chapterMap[def.chapterId] = {
                    chapterId: def.chapterId,
                    chapterTitle: topic?.chapters.find(c => c.id === def.chapterId)?.title || '',
                    sections: [],
                };
            }
            if (!chapterMap[def.chapterId].sections.includes(def.sectionId)) {
                chapterMap[def.chapterId].sections.push(def.sectionId);
            }
        }
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
            <SEO
                title={`定義一覧 | ${topic.title} | Math Explorer`}
                description={`${topic.title}の定義一覧。全${definitions.length}件の定義をチャプター別に確認できます。`}
                url={`/roadmap/${topic.id}/definitions`}
            />
                    ...
                    >
                        ...
                        <div className="mb-8">
                            ...
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <span className="text-sm text-slate-400 dark:text-slate-500 self-end mb-1">({definitions.length}件)</span>
                            </div>
                        ...
                        <style>{`
                            @media (max-width: 639px) {
                                .def-list-content .prose { font-size: 0.9rem; line-height: 1.6; }
                                .def-list-content .prose p, .def-list-content .prose li { font-size: 0.9rem; line-height: 1.6; }
                                .def-list-content .prose .katex { font-size: 1.05em; }
                            }
                        `}</style>

                        {/* Definition List - チャプター別に連続表示 */}
                        <div className="def-list-content space-y-10">
                                        ...
                                        <div className="prose prose-slate dark:prose-invert max-w-none">
                                            {sections.sort((a, b) => parseFloat(a) - parseFloat(b)).map(sectionId => (
                                                <SectionBoxExtractor
                                                    ...
                                                    chapterId={chapterId}
                                                    sectionId={sectionId}
                                                    filterBorderClasses={['border-blue-500']}
                                                />
                                            ))}
                                        </div>
                                    </div>
            ...
            <Link
                to={`/roadmap/${topicId}/definitions/quiz`}
                className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 inline-flex items-center px-6 py-3 rounded-full bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold transition-all shadow-lg hover:shadow-xl hover:scale-105"
            >
                クイズで確認
            </Link>
        </div>
