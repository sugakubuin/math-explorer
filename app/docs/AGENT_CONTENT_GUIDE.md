...
- **Syllabus Structure**: You MUST strictly adhere to the structure provided in the corresponding syllabus markdown file under `references/syllabus/`. Specifically:
  - Each **numbered item** (e.g., `- **1. 特異値・特異ベクトル**`) in the syllabus MUST become an `<h2>` heading in your content file, in the same order as the syllabus. 
  - **IMPORTANT: Do NOT include the number prefix in the `<h2>` heading itself.** (e.g. use `<h2>特異値・特異ベクトル</h2>`, not `<h2>1. 特異値・特異ベクトル</h2>`).
  - **IMPORTANT: Do NOT use LaTeX math delimiters (`$`) in `<h2>` headings.** Use simple plain text instead (e.g., use `<h2><InlineMath math="n" />-等分点</h2>` or `<h2>n-等分点</h2>` instead of `<h2>$n$-等分点</h2>`).
...
- **Complete Proofs & Proof Marks**: Whenever a Theorem, Proposition, Lemma, or Corollary is introduced, you MUST provide a complete and rigorous proof using `<ContentBox type="proof">`. 
  - **MANDATORY**: Ensure that proofs are comprehensive and complete, even if they become lengthy. Providing only a summary or outline is permitted ONLY when a full rigorous proof would require extensive background knowledge that falls far outside the current subject territory's scope. "Proof omitted" or "Proof is left as an exercise" is strictly forbidden.
...
- **Cross-Referencing**: When referring to a specific definition, proposition, theorem, or example by its number, use the English label (e.g., "Proposition X.X-X", "Definition Y.Y", "Example Z.Z") rather than the Japanese equivalent (e.g., "命題 X.X-X", "定義 Y.Y"). 
  ...
  - **Referencing Topics**: When referring to another topic, use the format `X-Y TopicName` (e.g., "2-3 実解析", "3-12 位相空間論II"). 
    - Do **NOT** use phrases like "Chapter 2-3" or "実解析（Chapter 2-3）" because the number represents the Topic ID, not a Chapter.
    - Do **NOT** use phrases like "Topic X-Y" or "TopicName (Topic X-Y)". Only the exact format `X-Y TopicName` is allowed.
